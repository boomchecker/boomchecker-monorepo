...
Table~\ref{tab:robustness_heldout} summarizes the desktop float32 reference for the corpus-level waveform-domain robustness stress test. The model retains high recall across all tested SNR levels, but precision is low and degrades further under heavier corruption. As a result, MCC remains moderate and declines from 0.54 at 30~dB to 0.46 at 5~dB. The float32 reference therefore fails mainly through false-positive inflation rather than missed artillery launches.

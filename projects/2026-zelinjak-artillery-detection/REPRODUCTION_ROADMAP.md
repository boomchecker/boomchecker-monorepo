...
| M6 | ESP32-S3 hardware (volitelné) | PROBÍHÁ — per-sample shoda <=1 LSB potvrzena; zbývá Release build | M5 + deska | 4–8 h |
...
**Stav:** PROBÍHÁ — build, flash a **živá UART validace hotové a úspěšné** (per-sample shoda <= 1 LSB); zbývá jen Release build + `idf.py size` (krok 6).
...
- Deska je připojena přes USB/IP passthrough (typicky WSL2 ↔ Windows host) — spojení je nestabilní: kontrolní operace (`esptool` flash, krátké request/response sekvence) fungují spolehlivě, ale udržovaný obousměrný datový tok (UART app data, boot log) nefunguje vůbec — nula bajtů oběma směry i po opakovaných pokusech (reset přes RTS, čerstvé otevření portu, přepnutí appky na jiný port). **Příčina potvrzena přímo v `dmesg`:** `vhci_hcd: urb->status -104` (ECONNRESET) s "unlink" událostmi korelujícími přesně s časem testů — tunel aktivně zahazuje sustained USB provoz, nejde o firmware ani o volbu portu (viz krok 5 níže).
...
5. [x] **Vyřešeno.** Průběh: smoke test (3 vzorky) i passivní poslech nejdřív vrátily 0 bajtů (appka na `UART_NUM_0` mluvila na jiný fyzický port, než byla připojená deska — potvrzeno v `sdkconfig`: `CONFIG_ESP_CONSOLE_UART_NUM=0`). **Firmware upraven** (`main.cpp`, `Kconfig.projbuild`, `CMakeLists.txt`): nový Kconfig přepínač `CONFIG_APP_DATA_LINK_USB_SERIAL_JTAG` (default `n`, zachovává původní chování), který při `y` přesměruje datový spoj na nativní USB-Serial/JTAG. Po přepnutí byla odpověď stále nulová — v `dmesg` se navíc objevil `vhci_hcd: urb->status -104` (ECONNRESET), potvrzující nespolehlivý USB/IP tunel pro sustained provoz. **Uživatel rebuildnul kontejner a přepojil desku na druhý USB-C port** (fyzický UART-to-TTL převodník, ne nativní USB-Serial/JTAG) — i po rebuildu se ECONNRESET vzor v `dmesg` objevoval už 10 minut po startu (potvrzuje, že jde o trvalý, nezávislý problém tunelu), ale flash i UART data tentokrát **prošly bez jediné chyby**.
   ...
   - Plný test split (171 vzorků) na 3 variantách (`clean`, `noise_snr30db`, `noise_snr5db`) — **0 chyb/no-response ze všech 513 volání**.
   - **Per-sample srovnání ESP32-int8 vs. opravený PC-int8 (seed 42, `generated/results/per_sample_seed42.csv`):**

     | Varianta | n | Max \|delta\| | Mean \|delta\| | Přehozené predikce |
     |---|---|---|---|---|
     | clean | 171 | 0,000006 | 0,000000 | 0 |
     | 30 dB | 171 | 0,003906 | 0,000023 | 0 |
     | 5 dB | 171 | 0,003906 | 0,000274 | 0 |

     `0,003906 = 1/256` = přesně **jeden kvantizační krok (1 LSB)** výstupu tohoto modelu (scale `0,00390625`, zjištěno v M1). Delta u clean (0,000006) je čistě artefakt zaokrouhlení na 4 des. místa v odpovědi z desky (`%.4f`), ne skutečný rozdíl. **Přes všech 513 testovaných vzorků (3 hladiny šumu) nula přehozených predikcí.**
   - **Rozhodující důsledek:** PC-int8 (opravená dequantizace, M4) a ESP32-int8 se na **stejném kanonickém modelu** shodují v rámci 1 LSB — přesně cíl stanovený níže v akceptačních kritériích. To znamená, že **PC-vs-ESP32 diskrepance z původního článku (R2#6, R3, R4-major1) nebyla způsobena inherentním rozdílem platforem** — když se použije stejný model a opravená metodika, platformy souhlasí. Diskrepance v původní Tabulce III musela vzniknout jinak (nekonzistentní metodika/model mezi PC a ESP32 běhy při přípravě článku) — doplňuje M1 nález (různé váhy float32 vs. firmware) o silný, přímo naměřený důkaz.
   - Výstupy: `generated/reports/esp32_per_sample_{clean,snr30,snr5}_test.csv`.
6. [ ] Release build (`CONFIG_COMPILER_OPTIMIZATION_PERF`): přeměřit latenci; `idf.py size` -> Flash/RAM čísla; ověřit nativní podporu všech ops v TFLM na ESP32-S3 (R1 III.C) — další krok, hardware je momentálně dostupné a funkční.

**Akceptační kritéria:**
- [x] Per-sample shoda <= 1 LSB — **splněno přesně** (max delta = 1 LSB, 0 přehozených predikcí, 513 vzorků přes 3 hladiny šumu).

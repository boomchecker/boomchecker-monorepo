...
- **Power**: 14 dBm (25 mW) conducted output is configured, ~13 dB below the 27 dBm ERP
  (500 mW) ceiling even before accounting for antenna gain/feedline loss. This margin is
  intentionally generous for a bring-up default. PR1 exposes no runtime power control (the
  profile is fixed at compile time); a later PR's `RadioConfig` (boomlink.md section 8.2)
  is where a legal-power ceiling needs to be enforced against operator-supplied values.

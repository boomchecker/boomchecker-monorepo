 ...
 *
 *          Copied from fw/common/boomlink/tests/c_test.h rather than shared,
 *          because sharing it would make these two packages depend on each
 *          other and neither should. The comments below record failures that
 *          package actually hit; they are kept because they are the reason the
 *          macros look the way they do, not decoration.
 *
 *          Not a framework and deliberately not one: these are stateful
 *          scenarios (push samples, step, expect a window, push more), and what
 *          they need is a way to keep going after a failed expectation so one
 *          run reports every broken step rather than only the first.
 ******************************************************************************
 */
#ifndef BOOMDETECT_TEST_H
#define BOOMDETECT_TEST_H

#include <stdio.h>

/* Failure and check counters, defined once per test binary via
   BD_TEST_STATE. */
extern int bd_test_failures;
extern int bd_test_checks;

#define BD_TEST_STATE \
  int bd_test_failures = 0; \
  int bd_test_checks   = 0

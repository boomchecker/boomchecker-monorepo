...
#
# bd_test.h's `return 1` is the only way any C test here can fail, and nothing
# exercised it, because the path only runs when a test is already failing.
# Change it to `return 0` and all three C binaries exit 0 forever while printing
# their failures to stderr, which ctest hides on success. The same harness in
# fw/common/boomlink was demonstrated failing exactly that way, with two real
# bugs green at the same time. This package copied the harness, and for a while
# copied its claim to be guarded without copying the guard.
#
...
# a negative test that passes for those reasons is worse than no test.
#
# Usage: check_bd_test.sh <selfcheck-binary>
set -eu
...
expect() {
  ...
  want_rc=$2
  want_text=$3
  # The command runs as its own statement, not inside an `if`: `set -e` is
  # suspended inside an `if` condition, so a crash would be indistinguishable
  # from a controlled failure.
  out=$(BOOMDETECT_SELFCHECK_MODE="$mode" "$binary" 2>&1) && rc=0 || rc=$?
...
if BOOMDETECT_SELFCHECK_MODE=require "$binary" 2>&1 | grep -q 'REQUIRE-DID-NOT-RETURN'; then

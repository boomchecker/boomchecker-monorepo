...
bool boomlink_encode_envelope(const boomlink_Envelope *envelope, uint8_t *buf, size_t buf_size,
  ...
  /* `header` is a singular message-type field, so proto3/Nanopb track its
     ...
     it here instead of letting every call site remember to check. */
...
bool boomlink_decode_envelope(const uint8_t *buf, size_t len, boomlink_Envelope *out_envelope) {
  *out_envelope = (boomlink_Envelope)boomlink_Envelope_init_zero;
  ...
  /* A wire-valid Envelope with no header is malformed at the application
     level: boomlink.md section 7 has every message carry one. Treat it the
     same as any other decode failure rather than handing the caller a
     "successfully decoded" value with a meaningless zeroed header. */
  return out_envelope->has_header;
}

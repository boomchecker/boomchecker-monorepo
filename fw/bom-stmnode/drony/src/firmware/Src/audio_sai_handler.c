...
void audio_sai_process_half_callback(const int16_t *p_dma_buf) {
    // Decimate 48 kHz -> 16 kHz by keeping every 3rd sample, converting to
    // float32 on the fly, straight into the circular buffer. Alias-safe only
    // because the upstream PDM->PCM chain (pdm_pcm.c, 101-tap FIR with 8 kHz
    // cutoff) already band-limits the signal below the 16 kHz Nyquist. Do not
    // reintroduce a short decimation FIR here: its passband droop measurably
    // corrupts the MFCC features and flips SVM decisions.
    uint32_t dec_len = DMA_HALF_BUF_SIZE / DECIMATION_FACTOR; // 512
    for (uint32_t i = 0; i < dec_len; i++) {
        circular_audio_buffer[write_idx] =
            (float32_t)p_dma_buf[i * DECIMATION_FACTOR] / 32768.0f;

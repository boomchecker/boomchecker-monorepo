...
static void wifi_init_credentials(void) {
  ...
  if (wifi_nvs_open(&handle) == ESP_OK) {
    ...
    if (nvs_get_str(handle, WIFI_NVS_STA_SSID, s_wifi_credentials.ssid,
                    &ssid_len) != ESP_OK) {
      s_wifi_credentials.ssid[0] = '\0';
    }

    if (nvs_get_str(handle, WIFI_NVS_STA_PASS, s_wifi_credentials.password,
                    &pass_len) != ESP_OK) {
...
static void wifi_init_ap_config(void) {
  ...
  strncpy(s_ap_ssid, "BOOMCHECKER", sizeof(s_ap_ssid) - 1);
  s_ap_enabled = true;
...
void set_wifi_mode(wifi_mode_t mode) { s_wifi_mode = mode; }

wifi_mode_t get_wifi_mode(void) { return s_wifi_mode; }

void set_wifi_connected(bool connected) { s_wifi_connected = connected; }

bool is_wifi_connected(void) { return s_wifi_connected; }

void set_wifi_configured(bool configured) { s_wifi_configured = configured; }

bool is_wifi_configured(void) { return s_wifi_configured; }
...
esp_err_t wifi_store_credentials(const char *ssid, const char *password) {
  ...
  strncpy(s_wifi_credentials.password, password,
          sizeof(s_wifi_credentials.password) - 1);
...
void set_ap_enabled(bool enabled) { s_ap_enabled = enabled; }

...
\newcommand{\thesis}[7]{%
  \begin{frame}{\badge{#1}{#2}\ #3}
    \textcolor{ink!65}{\small\faUserTie\ #4}\par\medskip
...
\thesis{tA}{A}{Acoustic Drone Detection on the STM32 Platform}
  {Bachelor \textperiodcentered\ supervisor Martin Maxa}
...
\thesis{tD}{D}{Web App for Data Collection \& Map Visualization}
  {Bachelor or master \textperiodcentered\ supervisor Jakub Svatoš}
...
\thesis{tE}{E}{IMU Integration and Orientation Algorithms}
  {Bachelor \textperiodcentered\ supervisor Kamil Herman}
...
\begin{frame}{Kanban on GitHub Projects}
  ...
  \begin{tikzpicture}
    ...
    \node[card=tE] at ([yshift=-6mm]c1.north) {IMU calibration\\\color{ink!60}@student-E};
    \node[card=tC] at ([yshift=-17mm]c1.north) {TDOA simulation\\\color{ink!60}@student-C};
    \node[card=tB,line width=1pt] at ([yshift=-6mm]c2.north) {SAI 4-ch PDM + DMA\\\color{ink!60}@student-B};
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.55\textwidth}
      ...
      \begin{itemize}
        \setlength{\itemsep}{2pt}
        \item one task = one \textbf{issue}: title, \textbf{acceptance criteria}, assignee
        \item move your cards yourself, and keep \emph{In progress} short
        \item PR description with \code{Closes \#N} closes the issue on merge
      \end{itemize}
    \column{0.40\textwidth}
      \centering\small
      \faDesktop\ \ \textbf{Live demo}\\[-1pt]
      {\scriptsize\color{ink!65}create issue, then branch, PR, merge, card moves to Done}
  \end{columns}
\end{frame}

% --------------------------------------------------
\begin{frame}{A good issue}
  \begin{columns}[c,onlytextwidth]
    \column{0.50\textwidth}
      \begin{block}{Example}
        \small
        \textbf{Record 4 PDM mics via SAI + DMA}\par\smallskip
        \scriptsize
        \textit{Why}: needed for DoA experiments.\par\smallskip
        \textit{Acceptance criteria}
        \begin{itemize}
          \item[{\faSquare[regular]}] 4 channels streamed to PC via \code{stm32node-cli}
          \item[{\faSquare[regular]}] no dropped DMA buffers over 10 min
          \item[{\faSquare[regular]}] inter-channel delay measured and documented
        \end{itemize}
      \end{block}
    \column{0.45\textwidth}
      \begin{itemize}
        \setlength{\itemsep}{5pt}
        \item \textbf{Small}: a few days of work, not a semester.
        \item \textbf{Checkable}: you know when it's done.
        \item \textbf{Written down}: results and dead ends go into the issue or the docs.
      \end{itemize}
      \medskip
      \screenshot{figs/projects-board.png}{0.28\textheight}{our GitHub Projects board}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.55\textwidth}
      \begin{enumerate}
        ...
        \item Create your first \textbf{issues} on the board.

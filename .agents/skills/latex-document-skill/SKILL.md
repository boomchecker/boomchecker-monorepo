...
description: >
  Universal LaTeX workflow skill for creating, compiling, and converting
  documents to professional PDF with optional PNG previews. Covers resumes,
  reports, papers, theses, posters, beamer slides, books, invoices, letters,
  exams, cheat sheets, and academic CVs. Supports charts/tables/figures,
  Mermaid/TikZ diagrams, citations (BibTeX/biblatex), multi-language/CJK,
  conditional content, fillable forms, mail merge from CSV/JSON, latexdiff
  version PDFs, and PDF operations (OCR, extraction, merge, watermark, metadata).
  Includes format conversion (Markdown/DOCX/HTML <-> LaTeX), PDF-to-LaTeX
  pipelines, and robust compile tooling with pdflatex/xelatex/lualatex,
  latexmk, texfot filtering, PDF/A, and verbosity controls.
---

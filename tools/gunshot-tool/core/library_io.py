...
def save_peak(
    ...
    label = metadata.get('label', 'unknown') or 'unknown'
    date = metadata.get('date', '')
    location = metadata.get('location', '')
    primary_id = _get_primary_id(event_type, metadata)

    dir_name = f"{date}_{location}" if location else date
    output_dir = os.path.join(output_root, event_type, dir_name, primary_id, f"{primary_id}_{label}")
    os.makedirs(output_dir, exist_ok=True)

    filenames = []
    for i, ch_audio in enumerate(channels):
        cropped = ch_audio[start:end]
        filename = f"{primary_id}_{label}_ch{i + 1}_uid-{uid}.wav"
        filenames.append(filename)
        data_int16 = (np.clip(cropped, -1.0, 1.0) * 32767).astype(np.int16)
        wav_write(os.path.join(output_dir, filename), sr, data_int16)

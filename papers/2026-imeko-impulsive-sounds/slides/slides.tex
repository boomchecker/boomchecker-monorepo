...
\begin{frame}{Why acoustic surveillance}
  ...
  \note{%
    ...
    high-risk situation, with an active shooter, they fall short. //
    {\color{accent}[CUT IF LATE]} A camera has to be pointed the right way. A guard
    has to be in the right place. //\par\medskip
...
\begin{frame}{The acoustic signature of a gunshot}
  ...
  \note<2>{%
    ...
    exists only when the bullet travels faster than sound.
    {\color{accent}[CUT IF LATE]} The bullet drags a cone of compressed air behind it,
    and that cone sweeps across the microphone. {\color{accent}[/CUT]} That is the
...
\begin{frame}{What the numbers say}
  ...
  \note{%
    ...
    Second, GTCC is consistently better --- every frame length, both classifiers.
    {\color{accent}[CUT IF LATE]} The gammatone filter bank resolves the low
    frequencies more finely, and that is exactly where the energy of a muzzle blast
    sits. //\par\medskip
